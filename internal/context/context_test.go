...
func TestStoreCreateAndLoad(t *testing.T) {
	...
	ctx, err := Load(filepath.Join(s.Root, "local"))
	if err != nil {
		t.Fatalf("Load err=%v", err)
	}
	if ctx.Name != "local" {
		t.Fatalf("Name=%q, want local", ctx.Name)
	}
	if ctx.Config.Server.Address != "127.0.0.1:9820" {
		t.Fatalf("Address=%q", ctx.Config.Server.Address)
	}
	if ctx.Config.Server.PublicKey != "aabbccdd" {
		t.Fatalf("PublicKey=%q", ctx.Config.Server.PublicKey)
	}
	if ctx.KeyPair == nil || ctx.KeyPair.Public.IsZero() {
...
func TestStoreCurrentAutoSet(t *testing.T) {
	...
	if ctx.Name != "first" {
		t.Fatalf("Current Name=%q, want first", ctx.Name)
	}
}
...
func TestStoreUse(t *testing.T) {
	...
	if ctx.Name != "b" {
		t.Fatalf("Current Name=%q, want b", ctx.Name)
	}
}
...
func TestServerPublicKey(t *testing.T) {
	...
	ctx, err := Load(filepath.Join(s.Root, "spk"))
	if err != nil {
		t.Fatal(err)
	}
	key, err := ctx.ServerPublicKey()
	if err != nil {
...
func TestServerPublicKeyInvalid(t *testing.T) {
	...
	ctx, err := Load(filepath.Join(s.Root, "badpk"))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := ctx.ServerPublicKey(); err == nil {
...
func TestStoreLoadByName(t *testing.T) {
	...
	ctx, err := s.LoadByName("myctx")
	if err != nil {
		t.Fatalf("LoadByName err=%v", err)
	}
	if ctx.Name != "myctx" {
		t.Fatalf("Name=%q, want myctx", ctx.Name)
	}
	if ctx.Config.Server.Address != "127.0.0.1:9820" {
		t.Fatalf("Address=%q", ctx.Config.Server.Address)
	}
