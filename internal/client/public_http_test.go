...
func TestPublicHTTPRegisterAndReadBack(t *testing.T) {
	...
	tarData := buildPublicReleaseTar(t, firmware.DepotRelease{
...
func buildPublicReleaseTar(t *testing.T, release firmware.DepotRelease, files map[string][]byte) []byte {
	t.Helper()
	...
	writePublicTarFile(t, tw, "manifest.json", manifest)
	for name, data := range files {
		writePublicTarFile(t, tw, name, data)
	}
...
func writePublicTarFile(t *testing.T, tw *tar.Writer, name string, data []byte) {
	t.Helper()
