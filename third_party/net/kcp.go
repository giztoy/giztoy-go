...
var (
	createKCP = func(conv uint32, user unsafe.Pointer) unsafe.Pointer {
		return unsafe.Pointer(C.ikcp_create(C.IUINT32(conv), user))
	}
	setKCPOutput = func(kcp unsafe.Pointer) {
		C.kcp_set_output((*C.ikcpcb)(kcp))
	}
)
...
type KCP struct {
	...
	mu        sync.Mutex
	outputPtr atomic.Pointer[outputHandler]
}
...
func NewKCP(conv uint32, output func([]byte)) *KCP {
	...
	kcpPtr := createKCP(conv, unsafe.Pointer(ud))
	if kcpPtr == nil {
		kcpRegistryMu.Lock()
		delete(kcpRegistry, id)
		kcpRegistryMu.Unlock()
		return k
	}

	k.kcp = (*C.ikcpcb)(kcpPtr)
...
func (k *KCP) SetOutput(output func([]byte)) {
	if output == nil {
		k.outputPtr.Store(nil)
		return
	}
	k.outputPtr.Store(&outputHandler{fn: output})
}
