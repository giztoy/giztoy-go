...
import { listWorkspaceTemplates, type WorkflowTemplateDocument } from "../../../../packages/adminservice";
...
export function WorkspaceTemplatesListPage(): JSX.Element {
  const { error, hasNext, items, loading, nextPage, pageNumber, prevPage, refresh } = useCursorListPage<WorkflowTemplateDocument>(
    async (query) => {
      const result = await expectData(listWorkspaceTemplates({ query }));
      return {
  ...
  return (
    <div className="space-y-6">
      <PageBreadcrumb items={[{ href: "/overview", label: "Overview" }, { label: "Workspace Templates" }]} />

      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">AI</div>
          <h1 className="text-3xl font-semibold tracking-tight">Workspace Templates</h1>
          <p className="max-w-3xl text-sm leading-6 text-muted-foreground lg:text-base">
            Declarative workflow specs that define reusable workspace layouts before they are instantiated as workspaces.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
          <div className="space-y-1">
            ...
            <CardDescription>Workflow template documents grouped by top-level kind and metadata.</CardDescription>
          </div>
        ...
        <CardContent className="space-y-4">
          ...
          {loading ? (
            ...
            <EmptyState description="Workspace template documents will appear here after they are created." title="No workspace templates" />
          ) : (
                ...
                <TableBody>
                  {items.map((template) => (
                    <TableRow key={template.metadata.name}>
                      ...
                      <TableCell>{template.apiVersion}</TableCell>
                      <TableCell className="max-w-[28rem] text-sm text-muted-foreground">{template.metadata.description?.trim() || "—"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
