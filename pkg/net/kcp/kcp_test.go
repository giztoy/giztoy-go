...
func TestKCP_SetOutput_ConcurrentWithOutputCallback(t *testing.T) {
	var countA atomic.Uint64
	var countB atomic.Uint64

	k := NewKCP(99, func(data []byte) {
		if len(data) > 0 {
			countA.Add(1)
		}
	})
	...
	go func() {
		...
		payload := []byte("race-test-payload")
	...
	// 收尾：确保至少有一次回调成功执行。
	k.SetOutput(cbA)
	_ = k.Send([]byte("final-payload"))
	k.Update(uint32(time.Now().UnixMilli()))
	k.Flush()

	if got := countA.Load() + countB.Load(); got == 0 {
		t.Fatal("expected at least one output callback invocation")
	}
}
