...
func TestPublicFiberAdapterServerInfo(t *testing.T) {
	...
	serverpublic.RegisterHandlers(app, serverpublic.NewStrictHandler(&serverpublic.PublicServer{
...
func TestPeerServerPublicRoundTrip(t *testing.T) {
	...
	go func() {
		serveErrCh <- server.servePublic(serverConn)
	}()
...
func TestPeerServerServeRequiresHandlers(t *testing.T) {
	server := &PeerServer{}

	err := server.Serve(&giznet.Conn{})
	if err == nil {
		t.Fatal("Serve should fail when handlers are missing")
	}
...
func TestFiberAppHandlerHidesPanicDetail(t *testing.T) {
	app := fiber.New(fiber.Config{DisableStartupMessage: true})
	...
	rec := httptest.NewRecorder()
	fiberAppHandler(app).ServeHTTP(rec, req)
