...
Options:
  ...
  --publish-result             Publish report to benchmark/net/SMOKE_RESULT.md or FULL_RESULT.md
...
run_one() {
	...
	local -a cmd
	cmd=(go test ./benchmark/net -run '^$' -bench "$BENCH_FILTER" -benchmem -count "$COUNT" -benchtime "$BENCHTIME")
