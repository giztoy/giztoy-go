...
This directory contains benchmark suites for `pkg/net` protocol stacks:
...
go test ./benchmark/net -run '^$' -bench . -benchmem

# Run one family only
go test ./benchmark/net -run '^$' -bench 'BenchmarkNet_KCP_' -benchmem
```
...
```bash
BENCH_NET_LOSS_RATES=0,0.01,0.05 go test ./benchmark/net -run '^$' -bench 'BenchmarkNet_KCPOverNoise_Throughput' -benchmem
```
...
Use `benchmark/run.sh` with system traffic shaping:

- Linux: `tc netem`
- macOS: `pfctl + dnctl`

```bash
./benchmark/run.sh --profile wifi --scale smoke
```
...
./benchmark/run.sh --profile all --scale smoke --no-system-netem --publish-result
```
