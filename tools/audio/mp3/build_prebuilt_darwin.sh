...
if [[ ! -f "${LAME_SRC_DIR}/configure" ]]; then
	echo "[build_prebuilt_darwin] error: missing lame source at ${LAME_SRC_DIR}" >&2
	echo "[build_prebuilt_darwin] hint: run 'git submodule update --init --recursive'" >&2
	exit 1
fi
...
echo "[build_prebuilt_darwin] built mp3 artifacts at ${OUT_ROOT}"
